...
// We use the iota keword to assign successive integer values to the constants.
const (
...
// Define a custom Logger type. This will hold:
// - the output destination that the log entries will be writen to,
// - the minimum severity level that log entries will be writen for,
// - a mutex for coordinating the writes.
type Logger struct {
	out      io.Writer
	minLevel Level
	mu       sync.Mutex
...
func (l *Logger) print(level Level, message string, properties map[string]string) (int, error) {
	// If the severity of the log entry is below the minimum severity for the
	...
	// Declare a line variable for holding the actual entry text.
	var line []byte
...
// Implement a Write() method on the Logger so that it satisfies the io.Write interface.
