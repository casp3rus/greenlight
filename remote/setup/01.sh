...
read -p "Enter password for the greenlight DB user: " DB_PASSWORD
...
# Add the new user (and give the sudo privileges).
...
# Configure the firewall to allow SSH, HTTP and HTTPS trafic.
ufw allow 22
ufw allow 80/tcp
ufw allow 443/tcp
ufw --force enable
...
su -i -u postgres -c "CREATE DATABASE greenlight"
su -i -u postgres psql -d greenlight -c "CREATE EXTENSION IF NOT EXISTS citext"
su -i -u postgres psql -d greenlight -c "CREATE ROLE greenlight WITH LOGIN PASSWORD '${DB_PASSWORD}'"
